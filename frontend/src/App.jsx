...
function PlatformView({ view, onNavigate }) {
  ...
  if (view === "store") {
    ...
    const isSizedApparel = ["Shirts", "Apparel"].includes(selectedProduct?.category);
    const isHat = selectedProduct?.category === "Headwear";
    const productColors = isSizedApparel ? shirtColors : isHat ? ["Black", "Northwestern Purple", "Navy", "White"] : ["White", "Black", "Northwestern Purple"];
    const productSizes = isSizedApparel ? ["S", "M", "L", "XL", "2XL"] : isHat ? ["Adjustable"] : ["Standard"];
    ...
    const productPreviewType = selectedProduct?.category === "Gifts" ? "mug" : selectedProduct?.category === "Headwear" ? "hat" : selectedProduct?.category === "Desk Decor" ? "cube" : selectedProduct?.category === "Stickers" ? "sticker" : selectedProduct?.category === "Magnets" ? "magnet" : "shirt";
    function chooseProduct(productId) {
      const product = data.products.find((entry) => entry.id === Number(productId));
      const apparel = ["Shirts", "Apparel"].includes(product?.category);
      const hat = product?.category === "Headwear";
      setMerchForm({ ...merchForm, productId: String(productId), size: apparel ? "L" : hat ? "Adjustable" : "Standard", color: apparel || hat ? "Black" : "White" });
    }
    return <main className="platform-main merch-page">
      ...
      {error && <div className="notice error">{error}</div>}
      {orderResult && <div className="checkout-confirmation-overlay" role="dialog" aria-modal="true" aria-labelledby="confirmation-title"><section className="checkout-confirmation"><div className="confirmation-check">✓</div><p className="eyebrow">Checkout complete</p><h2 id="confirmation-title">Your order is confirmed!</h2><p>Order <strong>#{orderResult.id}</strong> has been submitted and is ready for the production workflow.</p><div className="confirmation-status"><span>Current status</span><strong>{orderResult.status.replaceAll("_", " ")}</strong></div><div className="confirmation-items">{orderResult.items.map((item) => <article key={item.id}><img src={item.photoUrl} alt={item.photoTitle || "Ordered photograph"} /><div><strong>{item.productName}</strong><span>{item.color} · {item.size} · Qty {item.quantity}</span></div><b>${((item.unitPriceCents * item.quantity) / 100).toFixed(2)}</b></article>)}</div><div className="confirmation-total"><span>Order total</span><strong>${orderResult.total.toFixed(2)}</strong></div><div className="confirmation-actions"><button type="button" className="secondary" onClick={() => setOrderResult(null)}>Continue shopping</button><button type="button" onClick={() => { setAdminModule("Orders"); onNavigate("admin"); }}>View order in Admin CRM →</button></div><small>No payment was collected during this demonstration checkout.</small></section></div>}
      <form className="merch-studio" onSubmit={submitMerchOrder}>
        <section className="merch-builder">
          <div className="merch-step"><span>1</span><div><h2>Choose a product</h2><p>Select from apparel, drinkware, desk keepsakes, stickers, magnets, and more.</p></div></div>
          <label className="product-select">Product<select value={merchForm.productId} onChange={(event) => chooseProduct(event.target.value)}><option value="">Select a merchandise item</option>{data.products.map((product) => <option value={product.id} key={product.id}>{product.name} — ${product.price.toFixed(2)}</option>)}</select></label>
          {selectedProduct && <article className="selected-product-summary"><span>{selectedProduct.category}</span><div><strong>{selectedProduct.name}</strong><small>Personalize this item with an approved photograph.</small></div><b>${selectedProduct.price.toFixed(2)}</b></article>}
  ...
  return (
    <div className="workspace-shell">
      ...
      <section className="workspace-content">
        ...
        {view === "book" ? <BookingExperience /> : <PlatformView view={view} onNavigate={setView} />}
      </section>
    </div>
  );
}
